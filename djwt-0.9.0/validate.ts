...
import { encodeToString as convertUint8ArrayToHex } from 'https://deno.land/std@0.100.0/encoding/hex.ts'
...
function checkHeaderCrit (
  ...
  function checkReservedWords (crit: Required<Jose>['crit']) {
    ...
    ]).has(str))) { throw Error("the 'crit' list contains a non-extension header parameter") } else return crit
  }
  ...
  function checkCritHandlers (
    ...
    ) { throw Error('critical extension header parameters are not understood') } else return [crit, handlers]
  }
...
function validateJwtObject (
  ...
  if (typeof maybeJwtObject.signature !== 'string') { throw ReferenceError('the signature is no string') }
  if (
    !(
  ...
  if (isObject(maybeJwtObject.payload) && has('exp', maybeJwtObject.payload)) {
    if (typeof maybeJwtObject.payload.exp !== 'number') { throw RangeError("claim 'exp' is not a number") } else if (isExpired(maybeJwtObject.payload.exp)) { throw RangeError('the jwt is expired') }
  }
...
async function validateJwt (
  ...
  try {
    ...
    ) { return oldJwtObject } else throw Error("signatures don't match")
  } catch (err) {
